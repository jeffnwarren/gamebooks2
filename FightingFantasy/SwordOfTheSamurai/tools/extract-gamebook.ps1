param(
    [string]$PdfPath = "source\altered FF20 Sword of the Samurai OCR.pdf",
...
function ConvertFrom-OcrNumberToken {
    ...
    $number = [int]$value
    if ($number -lt 1 -or $number -gt 400) {
        return $null
    }
...
function Test-ChoiceEndingLine {
    ...
    $nextLooksLikeText = ($next -match '^[A-Z''"]|^(You|The|As|If|Roll|Do|In|After|Before|Having|Suddenly|There|This|With|Hasekawa|Tokugawa|Hattori)\b')
...
function Split-Sections {
    ...
    foreach ($line in $combinedLines) {
        ...
        if (Test-OcrTokenMatchesNumber $line.Text $nextNumber) {
            ...
            for ($future = $nextNumber + 1; $future -le ([Math]::Min(400, $nextNumber + 10)); $future++) {
    ...
    if ($current -ne $null) {
        Add-SectionRun $sections $current.Number 400 $current.Page @($currentLines)
    }
...
function Get-Choices {
    ...
    foreach ($pattern in $patterns) {
        foreach ($match in [regex]::Matches($Text, $pattern)) {
            ...
            if ($number -ge 1 -and $number -le 400 -and $number -ne $CurrentNumber -and -not $choices.Contains($number)) {
                $choices.Add($number)
            }
        }
    }
...
[void]$js.AppendLine("  title: `"Sword of the Samurai`",")
[void]$js.AppendLine("  sourcePdf: `"../source/altered FF20 Sword of the Samurai OCR.pdf`",")
[void]$js.AppendLine("  coverImage: `"../source/swordofthesamurai.jpg`",")
