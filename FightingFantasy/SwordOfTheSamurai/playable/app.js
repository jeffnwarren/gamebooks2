(function () {
  ...
  const storageKey = "sword-of-the-samurai-play-state";
  const defaultState = {
    current: "intro",
    showPdf: false,
    back: [],
  ...
  function scanChoiceText(text, currentNumber) {
    ...
    const turnPattern = /\b(?:turn|tum|tuin|tuln|furn|fum|hrm|rurn|go|return|continue)\s+(?:to|at)?\s*(?:paragraph|section)?\s*([0-9OoQIiLlAaEeSsBbGgqQjJzZ()%{}.,'"]{1,6})/gi;
    ...
    return Array.from(choices, ([target, label]) => ({ target, label })).slice(0, 8);
  }
  ...
  function linkify(text) {
    ...
    const turnPattern = /((?:turn|tum|tuin|tuln|furn|fum|hrm|rurn|go|return|continue)\s+(?:to|at)?\s*(?:paragraph|section)?\s*)([0-9OoQIiLlAaEeSsBbGgqQjJzZ()%{}.,'"]{1,6})/gi;
  ...
  function formatReadableText(text) {
    ...
    value = value.replace(/\b(Now,\s+will you|So,\s+will you|But will you|Which way will you go\?)\s+/gi, "$1:\n");
  ...
  const introHeadingNames = new Map([
    ...
    ["skill, stamina, luck and honour", "Skill, Stamina, Luck and Honour"],
    ...
    ["restoring skill, stamina, luck and honour", "Restoring Skill, Stamina, Luck and Honour"],
    ["restoring skill, stamina and luck", "Restoring Skill, Stamina and Luck"],
    ["skill", "Skill"],
  ...
  function isIntroArtifact(line) {
    ...
    if (/^(?:INITIAL|ITEMS OF TREASURE|EQUIPMENT CARRIED|NOTES|- PROVISIONS)$/i.test(line)) return true;
    if (/^(?:SKILL =|STAMINA =|LUCK =|HONOUR =|SKILL STAMIMGA|SEALE|ALLS|SKULL|iNETMAL|kei\.)/i.test(line)) return true;
    return /^[=_'"., |\\/()%\-]+$/.test(line);
  }
  ...
  function normalizeIntroText(text) {
    ...
    value = value
      ...
      .replace(/\bHONOUR scores You\b/g, "HONOUR scores. You")
  ...
  refs.rollHeroBtn.addEventListener("click", () => {
    ...
    state.stats = {
      ...
      luckInitial: luck,
      luck,
      honourInitial: honour,
      honour
    };
    refs.diceOutput.value = `Skill ${skill}, Stamina ${stamina}, Luck ${luck}, Honour ${honour}`;
    saveState();
    renderSheet();
  });
