...
PDF_PATH = ROOT / "source" / "altered FF38 Vault of the Vampire.pdf"
...
def marker_numbers(line: OcrLine) -> set[int]:
    ...
    return {value for value in values if 1 <= value <= 400}
...
def split_sections(lines: list[OcrLine]) -> tuple[dict[int, str], dict[int, int]]:
    sections: dict[int, list[str]] = {number: [] for number in range(1, 401)}
    pages: dict[int, int] = {number: 1 for number in range(1, 401)}
    ...
    for key in sorted(groups.keys(), key=lambda item: (item[0], 0 if item[1] == "L" else 1)):
        ...
        for index, line in enumerate(group):
            ...
            if expected <= 400 and likely_marker(line, expected, width):
                current = expected
            ...
            future = [number for number in candidates if expected < number <= min(400, expected + 4)]
            if future:
                current = min(future)
                pages[current] = line.page
                expected = current + 1
            ...
            if current is not None and current <= 400:
...
def section_references(text: str, current: int) -> list[int]:
    ...
    for match in re.finditer(r"\b(?:turn|go|return|continue)\s+(?:to|at)?\s*([0-9OIlSBAgq]{1,4})", text, re.I):
        ...
        if candidates:
            ...
            if 1 <= candidate <= 400 and candidate != current:
                refs.add(candidate)
    return sorted(refs)
...
def main() -> int:
    ...
    choices = sum(len(section_references(sections[number], number)) for number in range(1, 401))
    print(f"Sections with text: {recovered}/400")
