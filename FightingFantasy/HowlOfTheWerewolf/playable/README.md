...
Run `npm run check` from the project root for JavaScript syntax, book-data integrity, intro rendering, proofreader smoke coverage, intro/background OCR smoke checks, and golden browser fixtures. Use `npm run check:choices -- 1 10 100` to sample specific sections, `npm run report:graph` for choice graph warnings, `npm run report:ocr -- 25` for the highest-scoring OCR suspects, `npm run report:review` for a human-facing OCR review queue plus safe-fix candidates, `npm run fix:intro-ocr:dry` to preview the curated intro/background cleanup, `npm run fix:intro-ocr` to apply it, `npm run fix:safe-ocr:dry` to preview exact safe OCR replacements, `npm run fix:safe-ocr` to apply them, `npm run fix:turn-refs:dry`/`npm run fix:turn-refs` to normalize garbled "turn to N" phrases in section prose, `npm run fix:stat-glyph:dry`/`npm run fix:stat-glyph` to split glued stat blocks, normalize garbled stat labels, and strip stray page-art glyphs, and `npm run export:proofreading` to generate raw/readable text copies plus paste-sized chunks.

`fix:turn-refs` rewrites a turn-phrase only when its OCR'd target resolves to a number already in that section's stored `choices`, which is why it never changes the choice graph (`report:graph` is identical before and after). Alpha-only number tokens (e.g. "turn tail", "him is") and connector-less phrases are flagged in `proofreading/review/applied-turn-fixes.md` for human review instead of being rewritten. `fix:stat-glyph` leaves `§` and `£` in place (this book's OCR often uses them as the digits 5/1) and leaves stat values written as letters (`SKILL?`, `STAMINA S`) for PDF review; both tools write a full before/after report under `proofreading/review/`.
